      ...
      >
        ...
        <div className="p-6 md:p-8 pt-4 md:pt-6">
            ...
            <div className="flex flex-wrap gap-x-4 gap-y-2 mb-4">
              {(Array.isArray(project.github)
                ...
                <a
                  ...
                  className="inline-flex items-center gap-2 text-sm font-medium text-gray-600 dark:text-neutral-300 hover:text-sky-600 dark:hover:text-sky-400 transition-colors"
                >
          ...
          {project.features?.length > 0 && (
            <div className="mb-8">
              <h4 className="text-lg font-semibold text-gray-900 dark:text-white mb-5">대표 기능</h4>
              <div className="space-y-4">
                {project.features.map((feature, i) => (
                  <div
                    key={i}
                    className="rounded-xl border border-gray-100 dark:border-neutral-700/50 overflow-hidden flex flex-col md:flex-row"
                  >
                    {/* Feature GIF */}
                    {feature.gif && (
                      <div className="bg-gray-50 dark:bg-neutral-900/50 p-4 flex justify-center md:w-[280px] md:shrink-0">
                        <img
                          src={feature.gif}
                          alt={feature.title}
                    ...
                    {/* Feature text */}
                    <div className="p-4 md:flex md:flex-col md:justify-center md:flex-1">
          ...
          {project.takeaways?.length > 0 && (
            <div className="mb-8">
              <h4 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">회고 및 배운 점</h4>
              <ul className="space-y-2">
                {project.takeaways.map((t, i) => {
                  const isObject = typeof t === 'object' && t !== null;
                  const keyword = isObject ? t.keyword : null;
                  const description = isObject ? t.description : t;
                  return (
                    <li key={i} className="flex items-start gap-2 text-sm text-gray-600 dark:text-neutral-300">
                      <span className="text-sky-500 dark:text-sky-400 shrink-0">▹</span>
                      <span>
                        {keyword && (
                          <span className="inline-block px-1.5 py-0.5 mr-1.5 rounded text-sky-700 bg-sky-50 dark:text-sky-300 dark:bg-sky-500/10 font-medium">
                            {keyword}
                          </span>
                        )}
                        {description}
